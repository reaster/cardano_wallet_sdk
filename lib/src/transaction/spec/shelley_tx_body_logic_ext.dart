...
extension ShelleyTransactionBodyLogic on ShelleyTransactionBody {
  ...
  Coin _existingBalance({ShelleyTransactionOutput? changeOutput, required String policyId, required String name}) {
    ...
    if (policyId == '' && name == lovelaceHex) {
      ...
      ShelleyMultiAsset? multiAsset = changeOutput.value.multiAssets
          .firstWhere((m) => m.policyId == policyId, orElse: () => null as ShelleyMultiAsset);
      if (multiAsset == null) return coinZero;
      ShelleyAsset? asset = multiAsset.assets.firstWhere((a) => a.name == name, orElse: () => null as ShelleyAsset);
